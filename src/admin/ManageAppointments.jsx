...
export default function ManageAppointments() {
  ...
  console.log("ROLE:", role, "SUPERADMIN:", superadmin);
  ...
  useEffect(() => {
    async function getVeri() {
      if (superadmin) {
        // Superadmin: tüm randevuları çek
        const snapshot = await getDocs(collection(db, "randevular"));
        setRandevular(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
      } else {
        // Normal admin: sadece kendi salonunun randevuları
  ...
  return (
    <div className="ma-wrap">
      ...
      {!loading && (superadmin || salonId) && randevular.length === 0 && (
